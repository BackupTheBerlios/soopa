<?php
// $Id: forum.php,v 1.4 2005/07/31 18:23:50 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
...
class Forum extends XoopsObject {
    ...
    function &getModerators($asUname = false)
    {
        ...
        $moderators = array();
        $forum_moderators = $this->getVar('forum_moderator');
    ...
    function getSubForums()
    {
        ...
        while ($forum_data = $this->db->fetchArray($result)) {
            $forums = array_merge(
                ...
                array('forum_moderators' => $this->disp_forumModerators($forum_data['forum_moderator'])
                    ),
    ...
    function disp_forumModerators($valid_moderators = 0)
    {
        ...
        if ($valid_moderators === 0) {
            ...
            $moderators = array();
            $criteira = "(" . implode(',', unserialize($valid_moderators)) . ")";
            $member_handler = &xoops_gethandler('member');
...
class NewbbForumHandler extends XoopsObjectHandler
{
    ...
    function insert(&$forum)
    {
        ...
        if (!$forum->cleanVars()) {
            newbb_message("NewbbForumHandler::cleanVars error");
            return false;
        }
    ...
    function &getForumsByCategory($categoryid = 0, $permission = "", $asObject = true)
    {
		...
		if(!isset($forums_array[0])) return array();
