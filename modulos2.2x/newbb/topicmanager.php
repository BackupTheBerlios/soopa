<?php
// $Id: topicmanager.php,v 1.2 2005/07/17 17:02:33 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
    ...
    if ( $mode == 'move' ) {
		...
		if(count($categories)>0 && count($forums)>0){
			foreach(array_keys($forums) as $key){
	            ...
	            foreach ($forums[$key] as $forumid=>$forum) {
	                $box .= "<option value='".$forumid."'>-- ".$forum['title']."</option>";
					if( !isset($forum["sub"])) continue; 
		            foreach (array_keys($forum["sub"]) as $fid) {
		                $box .= "<option value='".$fid."'>---- ".$forum["sub"][$key]['title']."</option>";
	                }
	            }
			}
	    } else {
