<?php
// $Id: admin_permissions.php,v 1.2 2005/07/17 17:02:32 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
...
switch($action){
	case "template":
        ...
        foreach (array_keys($glist) as $i) {
            $selected = is_array($perm_template[$i])?array_keys($perm_template[$i]):array();
	...
	default:
		...
		$op_select->addOptionArray(array("no"=>_NONE, "template"=>_AM_NEWBB_PERM_TEMPLATE, "apply"=>_AM_NEWBB_PERM_TEMPLATEAPP));
		$opform->addElement($op_select);
		$opform->display();
