<?php
//////////////////////////////////////////////////////////////////////////////
// $Id: rmsrv_srvs.php,v 1.1 2005/07/23 22:30:38 mauriciodelima Exp $             //
// ------------------------------------------------------------------------ //
...
function b_show_rmsrv_service($options){
	...
	for ($i=1;$i<=$options[0];$i++){
		...
		$rtn = array();
		if ($options[1]){ $form = FormBuy($row['nombre'],$row['codigo'],$row['precio'],'srv'); }
		$rtn['form'] = $form;
...
function bedit_service($options){
	if ($options[1]){ $chk1 = "checked='checked'"; $chk2 = ''; }else{ $chk2 = "checked='checked'"; $chk1 = ''; }
	
	$form = _MI_BEDIT_SRVNUM."<br><input type=\"text\" name=\"options[]\" value=\"$options[0]\" /><br>";
