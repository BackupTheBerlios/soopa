<?php
// $Id: update.php,v 1.3 2005/08/08 23:45:41 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
...
function xoops_module_pre_update_system(&$module) {
    ...
    if ($oldversion < 203) {
        ...
        $profile_module->install(array(XOOPS_GROUP_ADMIN), array(XOOPS_GROUP_ADMIN, XOOPS_GROUP_USERS, XOOPS_GROUP_ANONYMOUS));
    }
    if ($oldversion < 205) {
    ...
    if ($oldversion < 207) {
        ...
        if (!$configcat_handler->delete($userconfigcategory)) {
            $module->setErrors("Could not delete user configuration category");
        }
    }
    if ($oldversion < 208) {
        $configcat_handler =& xoops_gethandler('configcategory');
        ...
        if (!$GLOBALS['xoopsDB']->query($sql)) {
            $module->setErrors('Could not change confcat_id properties');
        }
        $sql = "ALTER TABLE ".$configcat_handler->table." DROP PRIMARY KEY";
        if (!$GLOBALS['xoopsDB']->query($sql)) {
            $module->setErrors('Could not drop confcat primary key');
        }
        $sql = "ALTER TABLE ".$configcat_handler->table." ADD PRIMARY KEY ( `confcat_id` , `confcat_modid` )";
        if (!$GLOBALS['xoopsDB']->query($sql)) {
            $module->setErrors('Could not add new primary key');
        }
    }
    return true;
}
