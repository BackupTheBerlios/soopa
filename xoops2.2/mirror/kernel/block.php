<?php
// $Id: block.php,v 1.1 2005/08/02 18:18:29 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
...
class XoopsBlockHandler extends XoopsPersistableObjectHandler
{
    ...
    function assignBlocks() {
        ...
        foreach (array_keys($block_arr) as $i) {
            ...
            if (empty($bcachetime) || !$xoopsTpl->is_cached('db:'.$btpl, 'blk_'.$block_arr[$i]->getVar('instanceid'))) {
                $xoopsLogger->queries[$xoopsLogger->context] = isset($xoopsLogger->queries[$xoopsLogger->context]) ? $xoopsLogger->queries[$xoopsLogger->context] : array();
            ...
            if (!isset($show_cblock) && in_array($block_arr[$i]->getVar('side'), array(XOOPS_CENTERBLOCK_CENTER, XOOPS_CENTERBLOCK_LEFT, XOOPS_CENTERBLOCK_RIGHT))) {
                //backwards compatibility fix
                $show_cblock = 1;
                $xoopsTpl->assign('xoops_showcblock', 1);
            }
            elseif (!isset(${"show_".$sides[$block_arr[$i]->getVar('side')]."block"})) {
                ${"show_".$sides[$block_arr[$i]->getVar('side')]."block"} = 1;
                $xoopsTpl->assign('xoops_show'.$sides[$block_arr[$i]->getVar('side')].'block', 1);
            }
