<?php
// $Id: main.php,v 1.1 2005/08/02 18:19:34 mauriciodelima Exp $
//  ------------------------------------------------------------------------ //
    ...
    if ($op == 'showmod') {
        ...
        for ($i = 0; $i < $count; $i++) {
            ...
            switch ($config[$i]->getVar('conf_formtype')) {
                ...
                case 'user':
                include_once XOOPS_ROOT_PATH.'/class/xoopslists.php';
                $ele = new XoopsFormSelectUser($title, $config[$i]->getVar('conf_name'), $config[$i]->getConfValueForOutput());
                break;
                case 'user_multi':
                include_once XOOPS_ROOT_PATH.'/class/xoopslists.php';
                $ele = new XoopsFormSelectUser($title, $config[$i]->getVar('conf_name'), $config[$i]->getConfValueForOutput(), true, 10, true);
                break;
                case 'password':
                $myts =& MyTextSanitizer::getInstance();
